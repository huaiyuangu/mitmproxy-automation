...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_COMMAND(self):
        ...
        # api access
        print self.path, self.hostname
        if self.hostname is None and len(self.path) > 0 and self.path[0] == '/':
            try:
                f = None
                ...
                print self.path, self.command, self.path, self.request_version
                print 'exception:', e
            finally:
                if f: f.close()
...
if __name__ == '__main__':
    #sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /home/huaiyuan/python_auto/hulu/common/proxy.key -out /home/huaiyuan/python_auto/hulu/common/proxy.crt
    proxy = None
    # if not argv[1:]:
    #     proxy = AsyncMitmProxy()
    # else:
    proxy = AsyncMitmProxy(ca_file='proxy.crt')
